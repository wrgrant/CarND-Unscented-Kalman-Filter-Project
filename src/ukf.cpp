...
UKF::UKF() {
  ...
  // Don't change these; they are defined per hardware.
 ...
 * @param delta_t the change in time (in seconds) between the last
...
void UKF::Prediction(double delta_t) {
  ...
  // ------------------------------------------------------------------------------
  // Section 18
  // Create augmented sigma points (augmented means they also take noise into account).
  ...
  MatrixXd Xsig_pred_aug(n_aug_, 2*n_aug_+1);

  //create augmented sigma points
  Xsig_pred_aug.col(0) = x_aug;
  for (int i = 0; i< n_aug_; i++)
  {
    Xsig_pred_aug.col(i+1)       = x_aug + sqrt(lambda_+n_aug_) * L.col(i);
    Xsig_pred_aug.col(i+1+n_aug_) = x_aug - sqrt(lambda_+n_aug_) * L.col(i);
  }
  ...
  for (int i = 0; i< 2*n_aug_+1; i++)
  {
    ...
    double p_x = Xsig_pred_aug(0,i);
    double p_y = Xsig_pred_aug(1,i);
    double v = Xsig_pred_aug(2,i);
    double yaw = Xsig_pred_aug(3,i);
    double yawd = Xsig_pred_aug(4,i);
    double nu_a = Xsig_pred_aug(5,i);
    double nu_yawdd = Xsig_pred_aug(6,i);
...
void UKF::UpdateRadar(MeasurementPackage meas_package) {
  ...
  // -------------------------------------------------------------------------
  // Lesson 27 - Predict radar sigma points
